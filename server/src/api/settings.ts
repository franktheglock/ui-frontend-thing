...
const DEFAULT_SHARED_SETTINGS = {
  ...
  selectedProvider: "",
  systemPrompt:
    'You are a highly capable AI assistant. You excel at providing helpful, clear, and accurate information. When writing code, always use appropriate markdown formatting and specify the language.\n\nYou have access to tools (like web search). When a user asks for current facts, product comparisons, prices, benchmarks, rankings, specifications, you MUST call web search before answering. Do not answer from memory if the answer could depend on up-to-date information. If you are even slightly unsure, search first.\n\nCRITICAL CITATION RULE: When providing information from search results or external URLs, you MUST cite your sources inline using the format [source:n], where n is the 1-indexed number of the search result or URL read. This is required for EVERY fact or claim that comes from a tool result.\n\nExample: "The capital of France is Paris [source:1]. The Eiffel Tower was completed in 1889 [source:2]."\n\nFailure to include inline citations is a violation of your instructions.',
  maxTokens: 131072,
  temperature: 0.7,
  topP: 1,
...
function mergeSettings(
  base: typeof DEFAULT_SHARED_SETTINGS,
...
async function loadSharedSettings() {
  ...
  const row = (await db.get(
    "SELECT value, updated_at FROM app_settings WHERE id = ?",
    "global",
  )) as any;
