...
import { ImageGenerationSettings, ImageProviderConfig, ImageProviderId } from './types'
...
const DEFAULT_PROVIDERS: ImageProviderConfig[] = [
  ...
  },
  {
    ...
    enabled: false,
    baseUrl: process.env.OPENROUTER_IMAGE_BASE_URL || 'https://openrouter.ai/api/v1',
  ...
  },
  {
    ...
    enabled: false,
    baseUrl: process.env.GEMINI_IMAGE_BASE_URL || 'https://generativelanguage.googleapis.com/v1beta',
...
function mergeProviders(incoming?: ImageProviderConfig[]): ImageProviderConfig[] {
...
export function mergeImageSettings(incoming?: Partial<ImageGenerationSettings>): ImageGenerationSettings {
  return {
...
export async function loadImageSettings(): Promise<ImageGenerationSettings> {
  ...
  const row = await db.get('SELECT value FROM app_settings WHERE id = ?', 'image_generation') as { value?: string } | undefined
...
export async function saveImageSettings(settings: Partial<ImageGenerationSettings>): Promise<ImageGenerationSettings> {
  const db = await getDb()
